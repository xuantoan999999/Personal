        ...
        Cart.addItem(request.auth.credentials.id, product_id, quantity).then(function (resp) {
            return Cart.getCart(request.auth.credentials.id).then(function (cart) {
                ...
                cart.items.forEach((val, index) => {
                    let price = (val.product.id_promotion == null || !val.product.id_promotion.status) ? val.product.price : (val.product.id_promotion.type == 'PC' ? (val.product.price - (val.product.price * (val.product.id_promotion.value / 100)))
...
function applyCoupon(request, reply) {
    ...
    return orderHelper.checkCoupon({
        ...
        if (err.code === 'Limit')
            err.message = 'Bạn đã sử dụng hết số lần quy định của mã giảm giá này';
            ...
            if (err) {
                ...
                var saveAndReply = function () {
                    ...
                    return promise.then(function (resp) {
                        ...
                        resp.populate({
                            ...
                            //update times used coupon
                            if (doc.id_coupon) {
                                orderHelper.updateCoupon(doc.coupon.code, -1);
                            }
                ...
                var setShippingFee = function (configs) {
                    ...
                    return ShippingFee.findById(order.id_shipping_fee).exec().then(function (shipping_fee) {
                        ...
                        let total_prd_no_onsale = 0;
                        order.order_detail.forEach(function (detail, index) {
                            if (detail.id_promote.id == null) {
                                total_prd_no_onsale += detail.total;
                            }
                        });

                        if ((shipping_fee.type == 1 || shipping_fee == '1') && total_prd_no_onsale > configs.FreeShipConfig.Urban.value) //noi thanh
                            order.payment_info.info.shipping_fee = 0;
                        if ((shipping_fee.type == 2 || shipping_fee == '2') && total_prd_no_onsale > configs.FreeShipConfig.Suburb.value) //ngoai thanh
                            order.payment_info.info.shipping_fee = 0;
                    ...
                    return orderHelper.getConfigs().then(function (configs) {
                        if (order.delivery_time == 'CHIEU') {
                            if (configs.OrderDeleveryOnAffernoon.status) {
                                let total_tmp = 0; // tổng tiền các sản phẩm thuộc cate đc khuyến mãi onsale affternoon
                                order.order_detail.forEach(function (detail) {
                                    let cates_prd = [];
                                    cates_prd = detail.product.category.map(function (cat) {
                                        return ('' + cat._id);
                                    });
                                    if (_.intersection(cates_prd, configs.OrderDeleveryOnAffernoon.listCateApply).length > 0 && detail.id_promote.id == null && _.intersection(cates_prd, configs.OrderDeleveryOnAffernoon.listCateException).length == 0) {
                                        total_tmp += detail.total;
                                    }
                                })
                                if (configs.OrderDeleveryOnAffernoon.type == "MN") {
                                    ...
                                    order.total_pay -= ((configs.OrderDeleveryOnAffernoon.value / 100) * total_tmp);
                                }
                            }
                        }
                            ...
                            if (resp.success) {
                                if (configs.FirstOrder.status) {
                                    ...
                                    if (configs.FirstOrder.type == "MN") { // tien
                                        if (configs.FirstOrder.value > total_prd_no_onsale)
                                            order.total_pay -= total_prd_no_onsale;
                                        else
                                            order.total_pay -= configs.FirstOrder.value;
                                    }
                                    else { // phan tram
                                        order.total_pay -= total_prd_no_onsale * (configs.FirstOrder.value / 100);
                                    }
                                }
                            }
                ...
                if (order.id_coupon) {
                    return Coupon.findOne(({
                        $and: [{
                            ...
                            $or: [
                                { _id: order.id_coupon },
                                { 'code_group._id': order.id_coupon }
                            ]
                        }]
                    }), function (err, coupon) {
                        let money_coupon = 0;
                        if (coupon) {
                            return orderHelper.checkCoupon({ order: order, codeCoupon: request.payload.coupon_code, userId: request.auth.credentials.uid }).then(function (resp) {
                                money_coupon = resp.money_coupon;


                                // set coupon's info static to order
                                order.coupon.code = request.payload.coupon_code; // fix save info coupon single or group
...
function cfOrderAffernoon(request, reply) {
    orderHelper.getConfigs().then(function (resp) {
        ...
        request.log(['error', 'api-order', 'cfOrderAffernoon', err])
        return reply(Boom.badRequest('Error unknown!', err));
    });
}
