...
function getListProduct(option) {
    return function (request, reply) {
        ...
        }).select('id_promotion name qty_in_stock price email thumb promotion category').lean();
        promise.then(function (data) {
            reply(data);
        });
    }
...
function getCoupon(option) {
    return function (request, reply) {
        let opt = { status: 'active' };
        if (option == 'query') opt._id = request.query.coupon;
        if (option == 'coupon_order') opt._id = request.query.coupon_order;
        let promise = Coupon.findOne(opt);
        promise.then(function (data) {
            reply(data);
        });
    }
