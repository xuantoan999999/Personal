...
function category(request, reply) {
    ...
    asyncCao.parallel({
        ...
        banner_top_item: function (cb) {
            let promise = Banner.find(productUtil.createOptBanner('category', 'top', 'item', parrentCategory.slug))
...
function search(request, reply) {
    ...
    try {
        var getData = async(function () {
            ...
            if (findCategory) {
                if (findCategory.parrent_id) {
                    let parent_cate = productUtil.getCateParent(findCategory);
                    category = await(Category.findById(parent_cate._id).populate(productUtil.autoPopCate('sub_category')).lean());
                }
                else {
            ...
            let common_opt = productUtil.createOptDueDate({
                $or: [{
                    $and: option_product,
                },
                {
            ...
            products = await(Product.find(option_proccess).populate(productUtil.popuPromo()).lean());

            // Check if not find any product
            if (count_products == 0) {
                product_find = false;
                option_proccess = productUtil.createOptDueDate({
                    category: {
                        $in: productUtil.getCateSub(categoryRauCu)
                    },
                });
                if (filter.length > 0) {
                    option_proccess = productUtil.createOptDueDate({
                        ...
                        'tag_processing.id_tag': { $in: filter }
                    });
                }
                products = await(Product.find(option_proccess).populate(productUtil.popuPromo()).lean());
            }
                ...
                let search_category = await(Category.findById(first_product.category[0])
                    .populate(productUtil.autoPopCate('parent_category')).select('parrent_id').lean());
                let parent_cate = productUtil.getCateParent(search_category);
                let find_parent_cate = await(Category.findById(parent_cate._id).populate(productUtil.autoPopCate('sub_category')).lean())
        ...
        asyncCao.parallel({
            ...
            banner_top_item: function (cb) {
                let promise = Banner.find(productUtil.createOptBanner('category', 'top', 'item'))
...
function hightlight(request, reply) {
    ...
    try {
        ...
        asyncCao.parallel({
            ...
            banner_top_item: function (cb) {
                let promise = Banner.find(productUtil.createOptBanner('category', 'top', 'item', 'khuyen-mai'))
                    .sort({ order: 1 }).limit(1);
            ...
            products: function (cb) {
                let opt_and = { active: true };
                if (category) {
                    opt_and.category = {
                        $in: productUtil.getCateSub(category)
                    }
                }
                ...
                }).populate(productUtil.popuPromo()).lean();
...
function detail(request, reply) {
    ...
    asyncCao.parallel({
        product: function (callback) {
            let promise = Product.findOne(productUtil.createOptDueDate({ _id: id }))
                ...
                .populate({
                    ...
                    populate: productUtil.popuPromo(),
        ...
        let tmp_order = [];
        let list_pro_month = [];
        let parent_category = productUtil.getCateParent(result.product.category_list[0]);
        ...
        if (request.query.api) {
            ...
            meta.og_image = webUrl + productUtil.checkImgOld('/files/thumb_image/product_image/', result.product.thumb);
...
var getAndCountCategory = async(function (category) {
    ...
    let options = function (cates) {
        return productUtil.createOptDueDate({
            category: {
                $in: productUtil.getCateSub(cates)
            },
        })
    };
...
function asyncFuncProduct(option, category, filter, select) {
    return new Promise(function (resolve, reject) {
        if (filter.length > 0) {
        ...
        let options = _.extend(productUtil.createOptDueDate({
            category: {
                $in: productUtil.getCateSub(category)
            },
        }), option);
        if (select) {
            let promise = Product.find(options).populate(productUtil.popuPromo()).select(select).lean();
            promise.then(function (resp) {
                resolve(resp);
            })
        } else {
            let promise = Product.find(options).populate(productUtil.popuPromo()).select('name qty_in_stock made_in price view_unit slug thumb tag_processing tag_product id_promotion promotion')
                .lean();
            promise.then(function (resp) {
                resolve(resp);
            })
...
function getProcessingWithCountProduct(category) {
    return new Promise(function (resolve, reject) {
        let promise = Tag.find({ type: 'CN' }).select('name description type').populate({
            path: 'proccesing',
            match: productUtil.createOptDueDate({
                category: {
                    $in: productUtil.getCateSub(category)
