...
function find(request, reply) {
    ...
    if (request.query.deletedAt) {
        filters.deletedAt = { $ne: undefined };
    }
    else {
...
function register(request, reply) {
    ...
    UserHelper.createUser(request.payload, request.pre.userByPhone, function (err, resp) {
        ...
        acl.addUserRoles(resp._id.toString(), ["user", "customer"], function (err) {
            ...
            // Cập nhật thông tin đặt hàng cho user mua hàng rồi đăng ký sau
            if (request.payload.idOrder && request.payload.idOrder != '' && request.payload.customer && request.payload.customer.shipping_address.length == 1) {
                UserHelper.updateInfoOrder(request.payload.idOrder, resp);
            }
...
function login(request, reply) {
    ...
    acl.userRoles(representUser, function (err, roles) {
        ...
        if (roles) {
            ...
            let role = roles;
            let promise = User.findOne(optionLogin, 'customer email name status _id deletedAt phone dob avatar password').exec();
                    ...
                    user.compare(password, function (err, result) {
                        ...
                        if (result || isRegister) {
                            ...
                            redisClient.get(request.auth.credentials.id, function (err, credentials) {
                                ...
                                delete user.password;
                                reply({ token: token, account: user }).header('Authorization', token).state('token', token, cookieOptions);
                            })
                        }
                    });
                });
