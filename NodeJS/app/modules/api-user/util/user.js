...
module.exports = {
    ...
    restoreUserDeleted: restoreUserDeleted,
    /*Get document*/
    getUser: getUser,
...
// Cập nhật thông tin đơn hàng của user mua rồi đăng ký sau
function updateInfoOrder(idOder, user) {
    if (idOder && user) {
        Order.findOne({ id_order: idOder }).exec().then(function (order) {
            if (order) {
                order.payment_info.info.user_id = user._id;
                order.payment_info.info.id_shipping_address = user.customer.shipping_address[0]._id;
                order.save();
            }
        }).catch(function (err) {
        })
