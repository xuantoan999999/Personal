...
angular.module('auth').controller('AuthenticationController', ['$scope', '$http', '$location', '$window', 'Authentication',
    function($scope, $http, $location, $window, Authentication) {
        ...
        $scope.signin = function() {
            ...
            data.scope = 'admin';
            $http.post($window.settings.services.userApi + '/api/portal/login', data)
                .then(function(response) {
                    if (response.data.token) {
                        $window.location.href = window.portalprefix;
                    }
        ...
        $scope.signout = function() {
            $http.get($window.settings.services.userApi + '/api/portal/logout')
                .then(function(response) {
