...
function getAll(request, reply) {
    ...
    if (request.query.name && request.query.name.length > 0) {
        let filters_common = (JSON.parse(JSON.stringify(filters)))
        filters.push({
            name: new RegExp(request.query.name, 'i')
        });
        let option_name = { $and: filters };
        let promise = Product.find(option_name).select('name').sort({
            status: -1
        }).lean().then(function (resp_name) {
            let nin = resp_name.map(function (item) {
                return item._id
            });
            ...
            let option_slug = { $and: filters_common };
            let promise = Product.find(option_slug).select('name').sort({
                status: -1
            }).lean().then(function (resp_slug) {
                let list_product = resp_name.concat(resp_slug);
                let totalPage = Math.ceil(list_product.length / itemsPerPage);
                let dataSend = {
                    totalItems: list_product.length,
                    totalPage: totalPage,
                    currentPage: page,
                    itemsPerPage: itemsPerPage,
                    tagList: tagList,
                    categoryWithSub,
                    productBalance: request.pre.getProductBalance
                };
                let products_id = list_product.splice((page - 1) * itemsPerPage, itemsPerPage);
                let list_func = [];
                products_id.forEach(function (product) {
                    list_func.push(function (callback) {
                        let find_pro = Product.findById(product._id).populate('category_list tag_processing.id_tag tag_product.id_tag promotion unit').lean();
                        find_pro.then(function (item) {
                            callback(null, item);
                        })
                    })
                })
        ...
        let promise = Product.find(option).populate('category_list tag_processing.id_tag tag_product.id_tag promotion unit').populate({
            path: 'category_list',
            populate: productUtil.autoPopCate('parent_category')
        }).sort({
            status: -1
        }).lean().paginate(page, itemsPerPage, function (err, items, total) {
