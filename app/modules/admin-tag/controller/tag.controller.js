...
function getAll(request, reply) {
    ...
    Tag.find(options).sort('-createdAt').paginate(page, itemsPerPage, function (err, items, total) {
        if (err) {
...
function getProductByTag(request, reply) {
    ...
    let expire = request.params.expire ? request.params.expire : null;
    let options = {}
    // if (expire) {
    //     expire = moment(expire, ["MM-DD-YYYY", "YYYY-MM-DD"]).format('MM-DD-YYYY');
    // }
    if (type == "SP") {
        options = {
            'tag_product.id_tag': {
            ...
            // 'tag_product': { $elemMatch: { 'expire_date.endDate': { $lte: expire } } }
        };
    }
    ...
    if (type == "CN") {
        options = {
            ...
            // 'tag_processing': { $elemMatch: { 'expire_date.endDate': { $lte: expire } } }
        }
    }
    // if (!expire) {
    //     delete options['tag_product'];
    // }
    let promise = Product.find(options).populate({
        path: 'category_list',
        populate: ProductHelper.autoPopulateCate('parent_category')
    }).select('name tag_product tag_processing category ').lean();
