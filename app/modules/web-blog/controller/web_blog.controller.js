...
function listPostByTag(request, reply) {
    ...
    let promiseTag = Tag.find({ type: 'BL', type_blog: { $in: ['GB'] } }, { 'name': true, _id: true }).exec();
...
function getListPostTips(request, reply) {
    ...
    let promiseTag = Tag.find({ type: 'BL', type_blog: { $in: ['MV'] } }, { 'name': true, _id: true }).exec();
...
// TIN TỨC
// PATH: /tin-tuc
function listNewsPost(request, reply) {
    const Meta = request.server.plugins['service-meta'];
    var meta = JSON.parse(JSON.stringify(Meta.getMeta('tin-tuc')));
    let Domain = request.info.referrer.substring(0, request.info.referrer.indexOf('/tin-tuc'));
    let params = [];
    if (request.params.params)
    { params = request.params.params.split('*'); }

    let promiseTag = Tag.find({ type: 'BL', type_blog: { $in: ['TT'] } }, { 'name': true, _id: true }).exec();
    ...
    return Blog.find(options).sort({ createdAt: -1 }).populate('auth_id tags').paginate(page, itemsPerPage, function (err, items, total) {
        ...
        let dataRes = {
            ...
            items: items,
            totalPageSub2: totalPage - 2,
            totalPageSub1: totalPage - 1
        };

        return promiseTag.then(function (tags) {
            let data = { meta, imgBlogDir, Domain, data: dataRes, tags: tags, isFilterTag: isFilterTag, menu: { news: true }, class: { body_class: 'page-blog' }, tag: tag };
            return reply.view('web-blog/view/client/blog-news/list-news', data, { layout: 'web/layout' });
        }).catch(function (err) {
            request.log(['error', 'blog'], err);
            return reply.redirect('/error404');
        })
...
function getDetailNewsPost(request, reply) {
    const Meta = request.server.plugins['service-meta'];
    var meta = JSON.parse(JSON.stringify(Meta.getMeta('detail-article-gb')));
    let config = request.server.configManager;
    let id = request.params.articleId;
    ...
    return query.exec(function (err, post) {
        ...
        return promiseTag.then(function (tags) {
            let data = {
                ...
                post: post,
                meta: meta,
                menu: { news: true }, // cờ để check active menu và page góc bếp hay page banner
                class: { body_class: 'page-blog' },
                tags: tags
            };

            return reply.view('web-blog/view/client/blog-news/detail-news', data, { layout: 'web/layout' });
        }).catch(function (err) {
            request.log(['error', 'detail blog'], err);
            return reply.redirect('/error404');
        });
...
function getPolicyPage(request, reply) {
    ...
    if (request.params.namePage == 'robots.txt' || request.params.namePage == 'googleefbd7906bd9b9e6b.html' || request.params.namePage == 'sitemap.xml')
...
// PATH: /chuong-trinh-khuyen-mai/{ten-chupng-trinh}
